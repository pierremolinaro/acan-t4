...
ACAN_T4FD_Settings::ACAN_T4FD_Settings (const uint32_t inWhishedBitRate,
  ...
  const uint32_t dataBitRate = inWhishedBitRate * uint32_t (inDataBitRateFactor) ;
  if (mWhishedBitRate != inWhishedBitRate) {
    ...
    uint32_t bestDataTQCount = dataTQCount ; // Setting for slowest bit rate
    uint32_t BRP = kCANClockFrequency / dataBitRate / dataTQCount ;
  //--- Loop for finding best BRP and best TQCount
    while ((dataTQCount >= 5) && (BRP <= 1024)) {
    //--- Compute error using BRP (caution: BRP should be > 0)
      if (BRP > 0) {
        const uint32_t error = kCANClockFrequency - dataBitRate * dataTQCount * BRP ; // error is always >= 0
    ...
    //--- Compute error using BRP+1 (caution: BRP+1 should be <= 256)
      if (BRP < 256) {
        const uint32_t error = dataBitRate * dataTQCount * (BRP + 1) - kCANClockFrequency ; // error is always >= 0
        if (error < smallestError) {
          smallestError = error ;
          bestBRP = BRP + 1 ;
          bestDataTQCount = dataTQCount ;
    ...
    //--- Continue with next value of TQCount
      dataTQCount -= 1 ;
      BRP = kCANClockFrequency / dataBitRate / dataTQCount ;
    }
  //-------------------------- Set the BRP
  ...
  //--- Triple sampling ?
    mTripleSampling = (inWhishedBitRate <= 125000) && (mArbitrationPhaseSegment1 >= 2) ;
